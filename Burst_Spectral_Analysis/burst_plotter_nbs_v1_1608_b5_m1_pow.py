...
min_BBNorm_l=data['min_BBNorm_l']
max_BBNorm_l=data['max_BBNorm_l']
...
if fit_method == '1':
    ...
    dist = 0.4 # in units of 10 kpc
    
    radius = np.sqrt(dist**2.0*BB_Norm_l)
    radius_max = np.sqrt(dist**2.0*(BB_Norm_l+max_BBNorm_l))
    radius_min = np.sqrt(dist**2.0*(BB_Norm_l-(-1*min_BBNorm_l)))
    radius_minerr = radius-radius_min
    radius_maxerr = radius_max-radius
    radius_yerr = [radius_minerr,radius_maxerr]
    ...
    axarr[1].set_ylim(0.5,3.7)
    axarr[1].set_yticks([1.0,2.0,3.0])
    ...
    axarr[2].set_ylim(-0.5,9.5)
    axarr[2].set_yticks([4.0,8.0])
    ...
    axarr[2].errorbar(b_time, radius, xerr=exp_err, yerr=radius_yerr, linestyle='None',marker='.', label = 'BB Norm',ms=msize, elinewidth=eline,color='Black',)
    #axarr[2].plot([18.523,18.523],[0.01,1350],linestyle='--',color='Red',lw=2)
    axarr[2].grid(True)
    axarr[2].set_ylabel(r'R (km)' )
    #axarr[2].set_ylabel(r'R$^2$/D$^2_{10kpc}$' )

    
    axarr[3].errorbar(b_time, rchi, xerr=exp_err, linestyle='None', marker='.', label = 'BB Norm',ms=msize, elinewidth=eline,color='Black',)
    axarr[3].set_ylim(0.6,1.6)
    axarr[3].set_yticks([1.0,1.5,2.0])
    ...
    plt.xlim(1.3,max_time)
    
    
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'.pdf',orientation='landscape')
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'.png',orientation='landscape')
...
if fit_method == '3':
    ...
    plt.xlim(1.2,max_time)
    
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_'+mine+'_'+maxe+'_pow.pdf',orientation='landscape')
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_'+mine+'_'+maxe+'_pow.png',orientation='landscape')
    if not os.path.isdir(burst_folder+source_name+'/plots/'):
        print('No Plots folder found, creating a new one')
        os.makedirs(burst_folder+source_name+'/plots/')
    plt.savefig(burst_folder+source_name+'/plots/figure'+source_name+'_b'+bid+'_f'+fit_method+'_'+mine+'_'+maxe+'_pow.pdf',orientation='landscape')
    plt.savefig(burst_folder+source_name+'/plots/figure'+source_name+'_b'+bid+'_f'+fit_method+'_'+mine+'_'+maxe+'_pow.png',orientation='landscape')
...
if (fit_method == '2') or (fit_method == '4'):
    ...
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'_pow.pdf',orientation='landscape')
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'_pow.png',orientation='landscape')
    if not os.path.isdir(burst_folder+source_name+'/plots/'):
        print('No Plots folder found, creating a new one')
        os.makedirs(burst_folder+source_name+'/plots/')
    plt.savefig(burst_folder+source_name+'/plots/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'_pow.pdf',orientation='landscape')
    plt.savefig(burst_folder+source_name+'/plots/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'_pow.png',orientation='landscape')
