...
#burst_folder = '/Users/tolga/ownCloud/burst_characterization_v3/'
...
if fit_method == '1':
    ...
    axarr[0].plot([18.523,18.523],[0.01,150],linestyle='--',color='Red',lw=2) #--for red line
    ...
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_'+mine+'_'+maxe+'.pdf',orientation='landscape')
    plt.savefig(burst_folder+source_name+'/burst'+bid+'/figure'+source_name+'_b'+bid+'_f'+fit_method+'_'+mine+'_'+maxe+'.png',orientation='landscape')
...
if fit_method == '3':
    ...
    axarr[0].set_ylabel(r'Flux ($\times10^{-9}$)')
    axarr[0].set_ylim(-8.0,225.0)
    axarr[0].set_yticks([25.0,100.0,200.0])
    #axarr[0].plot([18.523,18.523],[0.01,180],linestyle='--',color='Red',lw=2)
    axarr[0].grid(True)
    axarr[0].set_title(r' '+'4U 1606-522 Burst:'+bid)

    axarr[1].errorbar(b_time, BB_kT_l, yerr=BBkT_err,xerr=exp_err,linestyle='None', marker='.', label = 'BB kT',ms=msize, elinewidth=eline,color='Black',)
    axarr[1].set_ylim(1.0,4.7)
    #axarr[1].plot([18.523,18.523],[0.0,160],linestyle='--',color='Red',lw=2)
    axarr[1].grid(True)
    axarr[1].set_ylabel(r'kT (keV)')

    axarr[2].errorbar(b_time, BB_Norm_l, xerr=exp_err, yerr=BBNorm_err, linestyle='None',marker='.', label = 'BB Norm',ms=msize, elinewidth=eline,color='Black',)
    axarr[2].set_ylim(-100.0,420.0)
    axarr[2].set_yticks([0.0,150.0,300.0])
    #axarr[2].set_ylim(min_range_norm-0.1*min_range_norm,max_range_norm+0.1*max_range_norm)
    #axarr[2].set_yscale('log')
    ...
    if not os.path.isdir(burst_folder+source_name+'/plots/'):
        print('No Plots folder found, creating a new one')
        os.makedirs(burst_folder+source_name+'/plots/')
        plt.savefig(burst_folder+source_name+'/plots/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'.pdf',orientation='landscape')
        plt.savefig(burst_folder+source_name+'/plots/figure'+source_name+'_b'+bid+'_f'+fit_method+'_pow_'+mine+'_'+maxe+'.png',orientation='landscape')
