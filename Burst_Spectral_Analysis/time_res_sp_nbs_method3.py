...
for i in range(len(sp_list)-1):
        ...
        if (initial_rstat < (spers_rstat.values[0]+3.50)/spers_dof.values[0]):
                ...
                plt.ylim(ymin,ymax)
                plt.savefig(burst_folder+sid+'_b'+bid+'_'+fit_method+'_full_comp'+mine+'_'+maxe+'.pdf',orientation='landscape', papertype='a4')
                plt.savefig(burst_folder+sid+'_b'+bid+'_'+fit_method+'_full_comp'+mine+'_'+maxe+'.png',orientation='landscape', papertype='a4')
                ...
                if (None in cparmins) == True or (None in cparmaxes) == True or (0 in cparmaxes) == True or (0 in cparmins) == True:
                        ...
                        min_BBNorm_l.append(0)
                        max_BBNorm_l.append(0)
                        fa_l.append(get_covar_results().parvals[2])
                        min_fa_l.append(0)
                        max_fa_l.append(0)
                        ...
                        fa_l.append(get_covar_results().parvals[2])
                        min_fa_l.append(get_covar_results().parmins[2])
                        max_fa_l.append(get_covar_results().parmaxes[2])
