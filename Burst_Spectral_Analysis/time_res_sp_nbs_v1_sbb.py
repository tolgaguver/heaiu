...
def create_plots(components, x_max, x_min, ymax, ymin):
    ...
    plt.savefig(burst_folder+sid+'_b'+bid+'_'+fit_method+'_full_sbb.pdf',orientation='landscape', papertype='a4')
    plt.savefig(burst_folder+sid+'_b'+bid+'_'+fit_method+'_full_sbb.png',orientation='landscape', papertype='a4')
    ...
    plt.ylim(ymin,ymax)
    plt.savefig(burst_folder+sid+'_b'+bid+'_'+fit_method+'_full_comp_sbb.pdf',orientation='landscape', papertype='a4')
    plt.savefig(burst_folder+sid+'_b'+bid+'_'+fit_method+'_full_comp_sbb.png',orientation='landscape', papertype='a4')
        ...
        # print(date_obsi)
            ...
            if (initial_rstat < (spers_rstat.values[0]+3.50)/spers_dof.values[0]):
                    ...
                    if (None in cparmins) == False and (None in cparmaxes) == False and (0 in cparmaxes) == False and (0 in cparmins) == False:
                            ...
                            is_all_zero = np.all(np.linalg.eigvals(matrix) > 0)
                            if is_all_zero == False:
                                print('Covariance Matrix is not Positive Definite Skipping Flux calculation')
                                continue
...
df.transpose()
#df.to_excel(burst_folder+source_name+'_sp_res_'+bid+'_'+fit_method+'_sbb.xlsx') 
#df.to_csv(burst_folder+source_name+'_sp_res_'+bid+fit_method+'_sbb.csv') 

sorted_df = df.sort_values(by='MJD_OBS_l')
sorted_df.to_excel(burst_folder+source_name+'_sp_res_'+bid+'_'+fit_method+'_sbb.xlsx') 
sorted_df.to_csv(burst_folder+source_name+'_sp_res_'+bid+fit_method+'_sbb.csv') 
